...
while true; do
    dialog --title " ATEC // GESTOR DE BACKUPS v2.0 " \
        ...
        9  "Estado do RAID 10" \
        "" "" \
        10 "Ver Agendamentos Automaticos" \
        11 "Criar Novo Agendamento" \
        12 "Remover Agendamento" \
        13 "Limpar Todos os Agendamentos" \
        "" "" \
        14 "Reconfigurar" \
        2>"$TEMP_FILE"
    ...
    case $escolha in
        ...
        9)  estado_raid ;;
        10) ver_agendamentos ;;
        11) criar_agendamento ;;
        12) remover_agendamento ;;
        13) limpar_todos_agendamentos ;;
        14) dialog --title " Reconfigurar " --editbox "$CONF_FILE" 12 55 2>"$TEMP_FILE"
            if [[ $? -eq 0 ]]; then
